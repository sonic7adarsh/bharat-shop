...
public interface StorefrontCartItemRepository extends JpaRepository<CartItem, Long> {
     ...
     */
    @Query("SELECT ci FROM StorefrontCartItem ci " +
     ...
     */
    @Query("SELECT ci FROM StorefrontCartItem ci LEFT JOIN FETCH ci.product " +
     ...
     */
    @Query("SELECT COUNT(ci) FROM StorefrontCartItem ci WHERE ci.cart.id = :cartId")
    Long countByCartId(@Param("cartId") Long cartId);
    
    /**
     * Sum total quantity in a cart
     */
    @Query("SELECT COALESCE(SUM(ci.quantity), 0) FROM StorefrontCartItem ci WHERE ci.cart.id = :cartId")
     ...
     */
    @Modifying
    @Query("DELETE FROM StorefrontCartItem ci WHERE ci.cart.id = :cartId AND ci.product.id = :productId")
     ...
     */
    @Modifying
    @Query("DELETE FROM StorefrontCartItem ci WHERE ci.cart.id = :cartId")
     ...
     */
    @Modifying
    @Query("UPDATE StorefrontCartItem ci SET ci.quantity = :quantity, ci.updatedAt = CURRENT_TIMESTAMP " +
           "WHERE ci.cart.id = :cartId AND ci.product.id = :productId")
     ...
     */
    @Query("SELECT CASE WHEN COUNT(ci) > 0 THEN true ELSE false END FROM StorefrontCartItem ci " +
           "WHERE ci.cart.id = :cartId AND ci.product.id = :productId")
