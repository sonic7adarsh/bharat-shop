...
public interface PageRepository extends TenantAwareRepository<Page> {
     ...
     */
    @Query("SELECT p FROM Page p WHERE p.active = true AND p.published = true AND p.tenantId = :tenantId AND p.deletedAt IS NULL ORDER BY p.sortOrder ASC")
    List<Page> findActivePublishedByTenantId(@Param("tenantId") UUID tenantId);
    
    /**
     * Find pages by type
     */
    @Query("SELECT p FROM Page p WHERE p.pageType = :pageType AND p.active = true AND p.published = true AND p.tenantId = :tenantId AND p.deletedAt IS NULL ORDER BY p.sortOrder ASC")
     ...
     */
    @Query("SELECT COUNT(p) > 0 FROM Page p WHERE p.slug = :slug AND p.tenantId = :tenantId AND p.id != :excludeId AND p.deletedAt IS NULL")
